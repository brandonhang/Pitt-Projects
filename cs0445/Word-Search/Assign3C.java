 ...
 *   word has been found first. Unlike Assign3.java, this
 *   program will search for all instances of a phrase in the
 *   word search. No command-line arguments are to be entered
 *   as all interactivity comes from within the program. Also
 *   has a contingency should the phrase be a single letter.
 *   
 */
...
public class Assign3C {
	...
	private static int totalCount;					// Integer holding the number of times a phrase was found
	...
	public static void main(String[] args) {
		...
		do {							// Begins word search loop
			...
			totalCount = 0;							// Sets total count to zero
				...
				for (int i = 0; i < rowSize; i++) {				// Nested loop searches every space in the 2-dimensional array
					for (int j = 0; j < colSize; j++)
						if (phrase.charAt(0) == puzzle.getLetter(i, j)) {		// Conditional if the puzzle letter matches the phrase
							...
							totalCount++;							// Increments the total count by 1
						}
				}
			}
					...
					for (int j = 0; j < colSize; j++)
						if (phrase.charAt(0) == puzzle.getLetter(i, j)) {		// Conditional if the puzzle letter matches the first phrase letter
							...
							if (phraseFound) {						// Conditional if the phrase was found
								phraseFound = false;				// Resets the found phrase boolean to false
								totalCount++;						// Increments the total number of found phrases by 1
			...
			if (totalCount > 0) {									// Conditional if the total count is greater than 0
				System.out.println("was found " + totalCount + " time(s)");				// Prints the number of times the phrase was found
				
				if (phrase.length() == 1) {					// Conditional if the phrase is a single letter
					System.out.print(phraseTokens.nextToken() + ":");		// Prints out the next word in the phrase
					for (int i = 0; i < totalCount; i++)					// Loops for the number of times the phrase was found
					...
					String[] wordArray = new String[phraseTokens.countTokens()];		// Creates a string array to hold phrase words
					
					for (int i = 0; i < wordArray.length; i++)				// Stores words from the phrase into the array
						wordArray[i] = phraseTokens.nextToken();
					
					for (int i = 0; i < totalCount; i++) {		// Loops for the number of times a phrase was found
						System.out.print("(" + (i + 1) + ") ");		// Prints the occurrence number of the phrase
						for (int j = 0; j < wordArray.length; j++) {
							System.out.print("\t" + wordArray[j] + ": ");				// Prints out the next word in the phrase
				...
				puzzle.printPuzzle();			// Prints out the word search puzzle with the found phrase(s) capitalized
			}
